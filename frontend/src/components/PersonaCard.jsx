...
const TRAIT_COLORS = {
  confidence: { bg: "bg-gradient-to-r from-yellow-600 to-yellow-500", text: "text-yellow-400" },
  empathy:    { bg: "bg-gradient-to-r from-pink-600 to-pink-500",   text: "text-pink-400"   },
  aggression: { bg: "bg-gradient-to-r from-red-600 to-red-500",    text: "text-red-400"    },
  humor:      { bg: "bg-gradient-to-r from-emerald-600 to-emerald-500",text: "text-emerald-400"},
};
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="h-1.5 rounded-full bg-forge-border/60 overflow-hidden">
        <div
          className={`h-full rounded-full ${colors.bg} opacity-85`}
          style={{ width: `${value}%` }}
        />
      </div>
    </div>
...
export default function PersonaCard({ persona, onDelete }) {
  ...
  return (
    <div className="forge-card p-5 flex flex-col gap-4 relative overflow-hidden bg-forge-card/35 backdrop-blur-md border border-forge-border/80 rounded-2xl shadow-md transition-all duration-300 hover:border-violet-500/50 hover:-translate-y-1 hover:shadow-xl hover:shadow-violet-950/20 animate-fade-up">
      {/* Glare border light */}
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="text-forge-muted hover:text-red-400 border border-transparent hover:border-red-900/30 hover:bg-red-950/20 transition-all p-1.5 rounded-xl cursor-pointer"
